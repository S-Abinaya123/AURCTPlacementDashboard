...
const FacultyHomePage: React.FC = () => {
  ...
  const [selectedDept, setSelectedDept] = useState<{
    highPackage: string | null;
    quizOverall: string | null;
    quizPerDept: string | null;
    placementPerDept: string | null;
  }>({
    highPackage: null,
    quizOverall: null,
    quizPerDept: null,
  ...
  useEffect(() => {
    const details = authService.getUserDetails();
    if (details.userId) setFacultyDetails(details as FacultyDetails);
    fetchLeaderboard();
  }, []);
  ...
  const openModal = (key: string, dept?: string) => {
    setSelectedDept(prev => ({ ...prev, [key]: dept || null }));
  };

  /* ================= SAFE CHART HANDLERS ================= */

  const handlePieClick = (key: string, data: any) => {
    const dept = data?.payload?.department;
    if (dept) openModal(key, dept);
  };

  const handleBarClick = (key: string, data: any) => {
    const dept = data?.activePayload?.[0]?.payload?.department;
    if (dept) openModal(key, dept);
  };

  if (loading) return <VerifyingUserLoading />;
    ...
    <div className="p-6">
      ...
      <div className="bg-gradient-to-r from-green-600 to-green-800 rounded-lg shadow-lg p-6 mb-6 text-white">
        <h1 className="text-3xl font-bold">
          Welcome{facultyDetails?.userName ? ` ${facultyDetails.userName}` : ''}!
        </h1>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-4 rounded shadow">
          <h2 className="font-semibold mb-2">Placements (Top Packages)</h2>

          {(() => {
            const data = Object.entries(
              leaderboard?.topThreeHighPackage?.reduce((acc: any, s) => {
                acc[s.department] = (acc[s.department] || 0) + 1;
                return acc;
              }, {}) || {}
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    ...
                    nameKey="department"
                    label={({ payload }: any) =>
                      `${payload?.department}: ${payload?.count}`
                    }
                    onClick={(d) => handlePieClick('highPackage', d)}
                  >
                    {data.map((_, i) => (
        ...
        {/* PIE 2 */}
        <div className="bg-white p-4 rounded shadow">
          <h2 className="font-semibold mb-2">Quiz Participants</h2>

          {(() => {
            const data = Object.entries(
              leaderboard?.topFiveQuizRankers?.reduce((acc: any, r) => {
                acc[r.department] = (acc[r.department] || 0) + 1;
                return acc;
              }, {}) || {}
            ).map(([department, count]) => ({ department, count }));
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    ...
                    onClick={(d) => handlePieClick('quizOverall', d)}
                  >
                    {data.map((_, i) => (
                      <Cell key={i} fill={COLORS[i % COLORS.length]} />
                    ))}
        ...
        <div className="bg-white p-4 rounded shadow md:col-span-2">
          <h2 className="font-semibold mb-2">Placements by Department</h2>

          {(() => {
            const data = Object.entries(
              leaderboard?.topFivePerDepartment || {}
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={data}>
                  ...
                  <Bar
                    dataKey="count"
                    fill="#F59E0B"
                    onClick={(d) => handleBarClick('placementPerDept', d)}
                  />
                </BarChart>
              </ResponsiveContainer>
            );
      ...
      <Modal
        ...
        title="High Package"
      >
        No Data UI (keep your existing modal UI)
      </Modal>
