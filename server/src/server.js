...
/* =================================
   DATABASE CONNECTION (Stateless)
================================= */
let isConnected = false;
const connectDB = async () => {
  if (isConnected) return;
  try {
    // Note: Cloudflare might need a connection string optimized for serverless
    await mongoose.connect(process.env.MONGO_URI);
    isConnected = true;
    console.log("MongoDB Connected");
  } catch (err) {
...
================================= */
// Cloudflare has no local disk for /uploads. 
// You must use an external service like Cloudinary or Cloudflare R2 for images.
...
app.get("/", (req, res) => {
  res.status(200).json({ success: true, message: "API Working on Cloudflare" });
});

/* =================================
   REMINDER LOGIC (FOR CRON)
================================= */
// We export this function so Cloudflare's Scheduled Trigger can call it.
export const checkAndSendReminders = async () => {
  ...
  try {
    ...
    const upcomingInterviews = await PlacementInterview.find({
      interviewDate: { $gte: tomorrow, $lte: tomorrowEnd },
      reminderSent: { $ne: true }
    });
...
/* =================================
   CLOUDFLARE EXPORT
================================= */
// Cloudflare DOES NOT use app.listen(). 
// It expects the express 'app' to be exported as the default.
export default app;